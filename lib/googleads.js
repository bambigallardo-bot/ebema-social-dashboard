...
// Google da de baja versiones periódicamente. Probamos varias y recordamos la que sirve.
const VERSION_CANDIDATES = process.env.GOOGLE_ADS_API_VERSION
  ? [process.env.GOOGLE_ADS_API_VERSION]
  : ["v21", "v20", "v19", "v18"];
let _gadsVersion = null;
...
async function gaqlSearch(query, attempt = 0) {
  ...
  const call = (ver, body) =>
    fetch(`https://googleads.googleapis.com/${ver}/customers/${cid}/googleAds:search`, {
      method: "POST",
      headers,
      body: JSON.stringify(body),
    });

  // Determina la versión de API vigente (un 404 = esa versión ya no existe).
  if (!_gadsVersion) {
    for (const ver of VERSION_CANDIDATES) {
      const probe = await call(ver, { query, pageSize: 1 });
      if (probe.status === 404) continue;
      _gadsVersion = ver;
      break;
    }
    if (!_gadsVersion) throw new Error("Google Ads: ninguna versión de API respondió (todas 404). Define GOOGLE_ADS_API_VERSION con la vigente.");
  }

  const rows = [];
  let pageToken;
  for (let p = 0; p < 20; p++) {
    const res = await call(_gadsVersion, { query, pageToken });
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      if (res.status === 404 && p === 0 && attempt < 2) { _gadsVersion = null; return gaqlSearch(query, attempt + 1); } // versión cambió; renegocia
