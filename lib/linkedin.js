...
export async function getLinkedin(months) {
  // Decisión: LinkedIn es MANUAL (no se pudo conseguir la API). Sin token devolvemos null para que
  // el dashboard use el modo manual editable y genere los gráficos con lo que cargue el CM.
  // Si algún día hay token válido en Vercel, se reactiva solo (vuelve a "AUTO · API").
  ...
  // Si el token está pero la API no devolvió nada real (vencido/sin permisos), cae a manual (null).
  const hasRealData =
    followersTotal != null ||
    Object.values(monthly).some((v) => v && (v.impressions != null || v.acquired != null || v.reactions != null)) ||
    Object.keys(bestByMonth).length > 0;
