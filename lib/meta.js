...
async function fbMetricTry(pageId, candidates, since, until) {
  const errors = {};
  for (const name of candidates) {
    try {
      const j = await metaGet(`/${pageId}/insights`, { metric: name, period: "day", since, until });
      const data = j.data || [];
...
async function getFacebook(months) {
  ...
  const FB = {
    ...
    fanAdds: ["page_fan_adds", "page_fan_adds_unique", "page_daily_follows_unique", "page_follows"],
  ...
  for (const m of months) {
    ...
    for (const [concept, cands] of Object.entries(FB)) {
      const got = await fbMetricTry(pageId, cands, m.since, iu);
      r[concept] = got.value;
      usedDbg[concept] = got.used || Object.values(got.errors)[0] || "sin data";
    }
    if (!fbDebug) fbDebug = usedDbg; // diagnóstico del primer mes: qué métrica se usó / por qué falló
