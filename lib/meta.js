...
async function getInstagram(months) {
  ...
  for (const m of months) {
    ...
    const batch = await igTotalsBatch(igId, ["reach", "views", "total_interactions", "profile_views", "website_clicks"], m.since, m.until);
    ...
    if (!batch.ok) {
      reach = await igTotal(igId, "reach", m.since, m.until);
      views = (await igTotal(igId, "views", m.since, m.until)) ?? (await igTotal(igId, "impressions", m.since, m.until));
      interactions = await igTotal(igId, "total_interactions", m.since, m.until);
      profileViews = await igTotal(igId, "profile_views", m.since, m.until);
      webClicks = await igTotal(igId, "website_clicks", m.since, m.until);
    } else if (views == null) {
      views = await igTotal(igId, "impressions", m.since, m.until); // algunas cuentas usan "impressions"
    }
    const newFollowers = await igSum(igId, "follower_count", m.since, m.until);
...
// Varias métricas de página en UNA llamada. Si una métrica está deprecada, la Graph API
// puede rechazar todo el lote; por eso se hace fallback individual. Devuelve { ok, out, error }.
async function fbBatch(pageId, metrics, since, until) {
  try {
    const j = await metaGet(`/${pageId}/insights`, { metric: metrics.join(","), period: "day", since, until });
    const out = {};
    for (const row of j.data || []) out[row.name] = (row.values || []).reduce((a, v) => a + (Number(v.value) || 0), 0);
    return { ok: true, out };
  } catch (e) {
    return { ok: false, out: {}, error: String((e && e.message) || e) };
  }
...
async function getFacebook(months) {
  ...
  const fbMetrics = ["page_impressions", "page_impressions_unique", "page_post_engagements", "page_fan_adds", "page_views_total"];
  const monthly = [];
  let fbDebug = null;
  for (const m of months) {
    const batch = await fbBatch(pageId, fbMetrics, m.since, m.until);
    if (!batch.ok && !fbDebug) fbDebug = batch.error;
    let v = batch.out;
    if (!batch.ok) {
      v = {};
      for (const mm of fbMetrics) v[mm] = await fbSum(pageId, mm, m.since, m.until);
    }
    const count = posts.filter((p) => p.monthKey === m.key).length;
    monthly.push({
      key: m.key,
      impressions: v.page_impressions ?? null, // "visualizaciones"
      reach: v.page_impressions_unique ?? null, // "espectadores"
      engagement: v.page_post_engagements ?? null, // "interacciones"
      fanAdds: v.page_fan_adds ?? null, // "nuevos seguidores"
      profileViews: v.page_views_total ?? null, // "visitas al perfil"
      posts: count,
    });
    await sleep(250);
  }
