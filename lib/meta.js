...
async function getFacebook(months) {
  ...
  try {
    ...
    for (const it of j.data || []) {
      ...
      const reachU = ins.find((m) => m.name === "post_impressions_unique")?.values?.[0]?.value ?? null;
      const impr = ins.find((m) => m.name === "post_impressions")?.values?.[0]?.value ?? null;
      posts.push({
        id: it.id,
        message: (it.message || "").slice(0, 140),
        date: it.created_time,
        permalink: it.permalink_url,
        reach: reachU ?? impr ?? null,
        impressions: impr,
        reachUnique: reachU,
  ...
  const FB = {
    impressions: ["page_impressions", "page_posts_impressions", "page_posts_impressions_organic"],
    reach: ["page_impressions_unique", "page_posts_impressions_unique", "page_posts_served_impressions_organic_unique"],
  ...
  for (const m of months) {
    ...
    if (!fbDebug) fbDebug = { pageToken: pageToken ? "ok" : pageTokenNote, ...usedDbg }; // diagnóstico del primer mes
