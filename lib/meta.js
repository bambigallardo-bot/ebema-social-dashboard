...
async function getFacebook(months) {
  ...
  for (const edge of ["published_posts", "posts", "feed"]) {
    try {
      const j = await metaGet(`/${pageId}/${edge}`, { fields: "message,created_time,permalink_url", limit: 50 }, 0, pageToken);
      const data = j.data || [];
  ...
  for (const p of posts.filter((p) => wantedKeys.has(p.monthKey)).slice(0, 40)) {
